...
public class OutboxEventPublishScheduler {
  ...
  public void processEvents() {
    ...
    for (var event : events) {
      try {
        var topic = resolveTopic(event.domainType());
        ...
        LOGGER.info("Published and deleted outbox event id={} to topic={}", event.id(), topic);
      } catch (Exception e) {
        LOGGER.error(
            "Failed to publish outbox event id={} domainType={}",
            event.id(),
            event.domainType(),
            e);
      }
  ...
  private String resolveTopic(DomainType domainType) {
    return switch (domainType) {
      case DRIVER -> driverEventsTopic;
    };
  }
}
